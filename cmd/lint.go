...
func init() {
	help = LintCmd.Flags().Bool("help", false, "print usage information")
	vflag = LintCmd.Flags().Bool("v", false, "print verbose information to standard error")
	dflag = LintCmd.Flags().Bool("d", false, "alias for -mode=diff")
	rflag = LintCmd.Flags().Bool("r", false, "find starlark files recursively")
	mode = LintCmd.Flags().String("mode", "", "formatting mode: check, diff, or fix (default fix)")
	format = LintCmd.Flags().String("format", "", "diagnostics format: text or json (default text)")
	diffProgram = LintCmd.Flags().String("diff_command", "", "command to run when the formatting mode is diff (default uses the BUILDIFIER_DIFF, BUILDIFIER_MULTIDIFF, and DISPLAY environment variables to create the diff command)")
	multiDiff = LintCmd.Flags().Bool("multi_diff", false, "the command specified by the -diff_command flag can diff multiple files in the style of tkdiff (default false)")
	lint = LintCmd.Flags().String("lint", "", "lint mode: off, warn, or fix (default off)")
	warnings = LintCmd.Flags().String("warnings", "", "comma-separated warnings used in the lint mode or \"all\"")
	filePath = LintCmd.Flags().String("path", "", "assume BUILD file has this path relative to the workspace directory")
	tablesPath = LintCmd.Flags().String("tables", "", "path to JSON file with custom table definitions which will replace the built-in tables")
	addTablesPath = LintCmd.Flags().String("add_tables", "", "path to JSON file with custom table definitions which will be merged with the built-in tables")
	version = LintCmd.Flags().Bool("version", false, "Print the version of buildifier")
	inputType = LintCmd.Flags().String("type", "auto", "Input file type: build (for BUILD files), bzl (for .bzl files), workspace (for WORKSPACE files), default (for generic Starlark files) or auto (default, based on the filename)")
}

var LintCmd = &cobra.Command{
	Use:   "lint",
	Short: "Lint a pixlet app or a directory of pixlet apps.",
	Run:   lintCmd,
}
