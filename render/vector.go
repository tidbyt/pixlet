...
func (v Vector) ToSkia(bounds image.Rectangle, frameIdx int) string {
	// (dx, dy) determines the orientation of this Vector
	dx, dy := 1, 0
	if v.Vertical {
		dx, dy = 0, 1
	...
	for i, cb := range childrenBounds {
		...
		child, ok := v.Children[i].(WidgetWithSkia)
		if !ok {
			continue
		}

		childSkia := child.ToSkia(image.Rect(0, 0, boundsW-dx*sumW, boundsH-dy*sumH), frameIdx)
		...
		fmt.Fprintf(&skia, `
			...
			canvas->restore();
			`,
			dx*offset+dy*crossOffset, dx*crossOffset+dy*offset,
			0, 0, cb.Dx(), cb.Dy(),
			childSkia,
		)
